...
//       target_ZMP = Pzmp - target_ZMP;
       target_ZMP[1]=Pzmp[1]-0.01*copysign(1.0,Pzmp[1]);
       target_ZMP[2]=Pzmp[2]-0.01*copysign(1.0,Pzmp[2]);
       target_ZMP[0]=0;
   ...
   if( now<timeLimit/*&& sqrt(error[2]*error[2])>=ep*/){
    double ts=0.02;
    double velMax=3;//[º/s]
...
//    double conDegToRad=3.14/180;
    ...
    double convDegreeToZMP=4.0;//1.0/2.0;//1.0/2.0;//4.0*0.7;//4.0/2;//[grados de giro del robot/m de ZMP]
    //x_pie=z_base
    //y_pie=-y_base
    //z_pie=x_base;
...
//    PID.AntiWindup(-limit,limit);
    controlSignal[0]=0.05*error[2]+0.002*(error[2]>filtro);
//    controlSignal[0]=0.0258*error[2]+0.00149*(error[2]>filtro);
    //positivo hace     que el robot gire a la izquierda en y es decir vaya hacia el lado positivo del frame de la base
//    controlSignal[1]= 2.17*error[1]+0.667*(error[1]>filtro2);
//   controlSignal[1]=PIDy.UpdateControl(error[1]);
   controlSignal[1]=error[1]*convDegreeToZMP;//*copysign(1.0,leftInitial[1]);
//yCDebug(OFS)<<copysign(1.0,leftInitial[1]);//<<leftInitial[0]<<leftInitial[1]<<leftInitial[2];
   //   controlSignal[0]=-error[2]/100;
    ...
    solver->invKin(leftInitial, previousJointPose, nextPosition);
...
bool OneFootStand::checkLimits(std::vector<double> &JointPose){


    for(int i=0;i<JointPose.size();i++){
         double qmax, qmin;
      if( !limits->getLimits(i,&qmin,&qmax) || (qmin>JointPose[i] || qmax<JointPose[i])){

          yCDebug(OFS) <<"N de articulacion: "<<i<<", Valor:"<<JointPose[i]<<", qmin="<<qmin<<", qmax="<<qmax;
          return false;
      }
    }
    return true;
